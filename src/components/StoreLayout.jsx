...
const StoreLayout = ({ layoutData = null }) => {
  ...
  const calculateSectionPosition = (section) => {
    // Check if the section is an aisle or a regular section
    if (section.number) {
      // This is an aisle
      return {
        x: section.posX,
    ...
    // For regular sections, find the maximum x and y
    let maxX = 0;
    let maxY = 0;

    // Find max position from other sections
    layoutData.sections.forEach((s) => {
      if (s.number) return; // Skip aisles
      const sectionEndX = s.posX + (s.cells ? Math.max(...s.cells.map(c => c.posX + c.width)) : 0);
      const sectionEndY = s.posY + (s.cells ? Math.max(...s.cells.map(c => c.posY + c.height)) : 0);
      maxX = Math.max(maxX, sectionEndX);
      maxY = Math.max(maxY, sectionEndY);
    });

    return {
      x: section.posX > maxX ? section.posX : maxX + 1,
      y: section.posY > maxY ? section.posY : maxY + 1,
    };
  };

  // Color mapping and other utility methods remain the same as in the original component
  const getColor = (status) => {
    switch (status?.toLowerCase()) {
      case "in-stock":
        return "#10B981";
  ...
  const calculateSectionDimensions = (section) => {
    // If it's an aisle, return fixed dimensions
    if (section.number) {
      return {
        width: section.width,
  ...
  const ShelfSection = ({ section, x, y }) => {
    // Skip rendering for aisle sections
        ...
        <Stage width={CANVAS_WIDTH} height={CANVAS_HEIGHT}>
          <Layer>
            ...
            {layoutData.sections.map((section, index) => {
              ...
              // If section has a number, it's an aisle
              if (section.number) {
                return (
                  <Aisle
                    key={section.id || index}
              ...
              // Otherwise, it's a regular section
              return (
                <ShelfSection
                  key={section.id || index}
                  section={section}
